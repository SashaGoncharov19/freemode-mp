...
impl ExecutableLoader {
    ...
    pub fn load_into_module(&mut self, target_module: HMODULE) -> std::result::Result<bool, String> {
        ...
        unsafe {
            ...
            let alloc_addr = windows::Win32::System::Memory::VirtualAlloc(
                None,
                self.load_limit,
                ...
                loop {
                    ...
                    if (thunk.address_of_data >> 63) == 0 {
                        ...
                        if let Ok(func_name) = std::str::from_utf8(&func_bytes) {
                            let resolved = GetProcAddress(loaded_module, windows_core::CStr::from_bytes_with_nul_unless(func_name.as_bytes()));
